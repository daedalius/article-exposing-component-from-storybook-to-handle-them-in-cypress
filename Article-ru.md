...
* Cypress показался нашей команде настолько мощным, что мы оставили в прошлом инструменты использующие js-dom под капотом.
...
* js-dom это синтетическая среда, ограничивающая охват покрытия. По сути проблемы везде за исключением JavaScript.
* Не очень выходит работать с js-dom так как это делал бы пользователь. Особенно когда речь заходит об имитации событий ввода. И поверьте мне, Cypress в этом деле непревзойден.  
...
Если вы пишете "низкоуровневые" юнит-тесты с покрытием деталей реализации и особенностей работы react-lifecycle - ... Не знаю. Я не пишу такой код. Вы уверены, что это уровень ответственности компонента? Может быть, стоит что-то вынести из него и тестировать отдельно?  
