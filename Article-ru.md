...
* Cypress показался нашей команде настолько мощным, что мы полностью отказадись от инструментов, использующих js-dom под капотом для тестирования UI-компонентов.
...
## Cypress
Я предпочитаю начинать работу над компонентом с перечисления тест-кейсов. После того как мы определились с тестовым покрытием, получаем следующую заготовку, который исполнит Cypress:
...
Честно говоря, я не писал такой тест прежде. Идея пришла в голову при написании статьи. Возможно, это будет полезно с точки зрения юнит-тестирования.

Сделать в Cypress это довольно просто. Достаточно предварительно создать ref на компонент. Для полноты картины стоит упомянуть, что ref даёт доступ к state и другим составляющим компонента. Однако мне эта возможность ни разу не потребовалась.
